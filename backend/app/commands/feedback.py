...
FEEDBACK_PROMPT = (
    ...
    "5. Show the draft inline (title + description) and call "
    "`quiz('Wyślij issue?', ['Tak, wyślij', 'Edytuj opis', 'Anuluj'])`.\n"
    "6. If 'Tak, wyślij' → call `submit_feedback_issue(title, description)` "
    "and reply with the returned `web_url`. If 'Edytuj opis' → wait for the "
    "user's edited text in the next turn, then re-confirm. If 'Anuluj' → "
